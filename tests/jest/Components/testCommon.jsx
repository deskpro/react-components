...
describe('>>> Buttons --- Common', () => {
  ...
  it('+++capturing Snapshot of Highlighter', () => {
    const renderedValue = renderer.create(<Highlighter highlight="feedback">
        Android feedback (Support)
                                          </Highlighter>).toJSON();
    expect(renderedValue).toMatchSnapshot();
  });
  it('+++capturing Snapshot of List', () => {
    const renderedValue = renderer.create(<List className="test-class">
  ...
  it('+++capturing Snapshot of QueryableList', () => {
    const renderedValue = renderer.create(<QueryableList className="test-class" whereName="@all">
      <ListElement name="item1">
          Item1
      </ListElement>
      <ListElement name="item2">
    ...
    </QueryableList>).toJSON();
    expect(renderedValue).toMatchSnapshot();
  });
  it('+++capturing Snapshot of SelectableList', () => {
    const renderedValue = renderer.create(<SelectableList className="test-class">
      <ListElement name="item1">
          Item1
      </ListElement>
      <ListElement name="item2">
    ...
    </SelectableList>).toJSON();
    expect(renderedValue).toMatchSnapshot();
  });
  it('+++capturing Snapshot of ToggleableList', () => {
    const renderedValue = renderer.create(<ToggleableList className="test-class" on="click" toggle="selected">
      <ListElement name="item1">
          Item1
      </ListElement>
      <ListElement name="item2">
    ...
    </ToggleableList>).toJSON();
    expect(renderedValue).toMatchSnapshot();
  });
  it('+++capturing Snapshot of Popper', () => {
    const renderedValue = renderer.create(<Popper target="target" placement="bottom" opened>
      <p>Test</p>
    </Popper>).toJSON();
    expect(renderedValue).toMatchSnapshot();
  });
